...
const fetchUserData = async () => {
  try {
    const userData = await UserService.getUser(authStore.getUserId);
...
const updateProfile = async () => {
  try {
    ...
    formData.append("Id", authStore.userId);
    ...
    if (updatedUser) {
      ...
      userStore.updateUserPhoto(updatedUser.photoUrl);
      toast.success("Twoje dane zostały pomyślnie zaktualizowane!");
      router.push("/profile");
    }
  } catch (error) {
